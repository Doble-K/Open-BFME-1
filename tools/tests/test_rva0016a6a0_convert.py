#!/usr/bin/env python3
"""Prove the 0x0016A6A0 byte-getter conversion is clean C++ and byte-matches retail.
...
SOURCE = ROOT / "Code" / "GameEngine" / "Source" / "Common" / "TinyByteFieldGetters_16A6A0.cpp"
...
def test_source_is_clean_cpp():
    ...
    assert "BFME_BYTE_FIELD_GETTER( Rva0016A6A0, 0x326 )" in text
    print("PASS clean C++ body (no naked/asm/emit)")


def test_build_py_matches_retail():
    text = SOURCE.read_text(encoding="utf-8")
    n = text.count("BFME_BYTE_FIELD_GETTER( Rva")
    ...
    print("PASS build.py matches retail 0x0016A6A0")
