#!/usr/bin/env python3
"""Prove the 0x00710D20 dword-getter conversion is clean C++ and byte-matches retail.
...
SOURCE = ROOT / "Code" / "GameEngine" / "Source" / "Common" / "TinyDwordFieldGetters_710D20.cpp"
...
def test_source_is_clean_cpp():
    ...
    assert "BFME_DWORD_FIELD_GETTER( Rva00710D20, 0xF0 )" in text
    print("PASS clean C++ body (no naked/asm/emit)")


def test_build_py_matches_retail():
    text = SOURCE.read_text(encoding="utf-8")
    n = text.count("BFME_DWORD_FIELD_GETTER( Rva")
    ...
    print("PASS build.py matches retail 0x00710D20")
