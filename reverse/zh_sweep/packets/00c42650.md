...
- best Zero Hour candidate agrees on 87.0% of the bytes outside relocation sites: 47 of
  54 compared byte(s) match
- the other 36 byte(s) of its 90-byte body are relocation slots, blanked before
  comparing, so which global and which callee this body uses never entered that figure —
  the sweep's own 92.2% counts every blanked byte as agreeing
...
- `$L120997`
  in `reference/CnC_Generals_Zero_Hour/GeneralsMD/Code/GameEngineDevice/Source/W3DDevice/GameClient/Drawable/Draw/W3DModelDraw.cpp` (87.0% of 54 compared bytes)
