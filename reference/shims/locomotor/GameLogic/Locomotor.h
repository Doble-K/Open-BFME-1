...
//   ZH's ctor stores TWO vtable pointers ([this+0]=MemoryPoolObject vtbl,
//   [this+4]=Snapshot vtbl from the second polymorphic base), pushing
//   m_template to +0x8. Retail's ctor (RVA 0x1B60E0) stores exactly ONE
//   vtable pointer at [esi+0] and then writes the ctor's tmpl param straight
//   to [esi+4] (`mov [esi+4],ecx` at ctor+0x6b) -- i.e. retail Locomotor is
//   NOT polymorphically a Snapshot (crc/xfer/loadPostProcess are called only
//   through concrete Locomotor*/LocomotorSet*, never via a Snapshot* list,
//   so BFME dropped the second base). Every OVERRIDE-getter body sampled
//   (getRollDamping@0x413180, getRudderCorrectionDegree@0x413210,
//   LocomotorSet::findLocomotor@0x1B5CF0, three siblings @0x1B5970/
//   0x1B59A0/0x3FE740) uses `mov eax,[ecx+4]` for m_template, never +8.
//   Dropping the Snapshot base here reproduces that single-vptr layout and
//   shifts every subsequent Locomotor member down by 4 bytes to match,
//   uniformly (proven for LocomotorSet::findLocomotor, landed clean C++ via
//   this shim in LocomotorAccessors.cpp).
//
//   NOT fixed here: LocomotorTemplate has its OWN, separate field-layout
//   drift downstream of m_surfaces (which matches ZH's compiled +0x10, no
//   fix needed there). getRollDamping/getRudderCorrectionDegree still only
//   land as MASM: with m_template@+4 corrected, our compile puts
//   m_rollDamping at LocomotorTemplate+0x7c but retail reads +0x98 (a 0x1C
//   gap unrelated to the Locomotor vtable fix above) -- LocomotorTemplate
//   itself needs its own shim/RE pass before those two (and any other
//   OVERRIDE Real-getter) can go clean C++.
class Locomotor : public MemoryPoolObject
{
