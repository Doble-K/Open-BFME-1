...
private:
	...
	// disableRadar@0xCC1C0 and enableRadar@0xCC2C0 both read m_radarDisabled at
	// [this+0x60], m_disableProofRadarCount at [this+0x5c] and m_radarCount at
	// [this+0x58], while this header's natural layout (proven via an offsetof
	// probe against the unmodified reference header) puts them at 0x4c/0x48/0x44 -
	// a uniform +0x14 (5 dwords) inserted somewhere at/after m_playerIndex.
	// getPlayerMask@Player (matched, Object.cpp) proves m_playerIndex itself is
	// UNSHIFTED at [this+0x24] (same as this header's natural offset), so the
	// insertion is bounded to the m_side..m_upgradeList span (natural 0x28-0x40)
	// with no further evidence to place it more precisely within that span; the
	// exact sub-field is unresolved, but nothing in this TU touches m_side/
	// m_baseSide/m_playerType/m_money/m_upgradeList by name, so a single opaque
	// pad placed immediately before m_radarCount is sufficient and correct for
	// every field this TU actually reads.
	UnsignedInt									_bfme_radarClusterPad[5];			///< @0x44 unresolved BFME-only insertion (5 dwords)

	Int													m_radarCount;									///< @0x58 # of facilities that have a radar under the players control
	Int													m_disableProofRadarCount;			///< @0x5c # of disable proof radars.  A disable proof one will be in both refcounts
	Bool												m_radarDisabled;							///< @0x60 The radar is disabled regardless of the number of radar objects
