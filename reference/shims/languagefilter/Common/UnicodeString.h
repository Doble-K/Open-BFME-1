...
// BFME keeps UnicodeString's copy ctor out of line (the retail body at
// 0x888400, folded with StringBase<G>'s copy ctor). Declaring it without a
// body makes the temporary opaque, and MSVC then schedules the EH
// registration of a by-value temporary AFTER loading `this` instead of
// before it, which is the one shape difference in map::operator[].
// Declaring the copy ctor as a visible delegation to StringBase<G> -- the
// same two-instruction call retail makes -- lets the compiler order the two
// retail's way. TU-scoped: only LanguageFilter.cpp opts into this shim.
template <typename T>
class StringBase;
...
{
	((StringBase<wchar_t>*)this)->StringBase<wchar_t>::StringBase(*(const StringBase<wchar_t>*)&stringSrc);
}
