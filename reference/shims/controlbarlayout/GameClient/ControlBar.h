...
protected:
	...
	// BFME puts the six toggle-button images at this+0x2A0..0x2B4:
	// ControlBar::setUpDownImages @0x49D160 reads all six there, where this
	// header puts them at +0x2E0..+0x2F4 - a uniform -0x40. Their offsets are
	// what is proven; moving the block up displaces the 0x18 bytes below by
	// the same amount, and where THOSE five members really live in BFME is
	// not known - they simply take the space the toggle block vacated so
	// everything from m_transitionHandler onward keeps its offset.
