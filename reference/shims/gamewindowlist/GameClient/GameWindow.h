...
protected:
  ...
  // user defined callbacks
	...
	// BFME's sibling-list pointers sit 0x10 further into GameWindow than ZH's:
	// GameWindowManager::~GameWindowManager @0x483810 walks the list reading
	// m_next at [win+0x1F8] where this header puts it at +0x1E8.
	//
	// The 0x10 goes HERE, immediately in front of the pointers, because
	// everything earlier in the class is already right: seven accessors in
	// this same TU are matched against the unshimmed layout and pin it -
	// setTooltipDelay writes m_instData.m_tooltipDelay at +0x1C4 and the six
	// win*Color accessors index their array from +0x48. reference/shims/
	// gamewindow spreads the same 0x10 differently (a BFME-only pointer at
	// +0x04 plus three more later), which moves both of those and breaks all
	// seven; that shim's +0x04 field can only coexist with them if something
	// before +0x48 is 4 bytes SMALLER in BFME, which nothing here proves.
	char _bfme_pad_pre_sibling_list[0x10];
	GameWindow *m_next, *m_prev;	// retail +0x1F8, +0x1FC
