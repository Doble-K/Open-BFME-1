...
// SHIM: BFME's ArchiveFileSystem does not derive from SubsystemInterface.
// Slot 2 of its vtable at 0x01143A08 is _purecall, where every one of the 147
// real SubsystemInterface vtables has ?loadIniFilesFromLegend@SubsystemInterface@@
// -- so it is not one of them. The layout says the same thing: openFile reaches
// m_archiveFileMap at this+4, i.e. straight after the vptr, which only works if
// no base contributes data. With the reference base it lands at this+8.
