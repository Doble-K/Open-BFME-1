...
// -----------------------------------------------------
// SHIM: AsciiString(const char*) left undefined so it emits a call to
// ??0AsciiString@@QAE@PBD@Z (retail 0x00888BC0). BFME's is a thin body; the
// reference inline expands to strlen + ensureUniqueBufferOfSize at every site.
...
// ?releaseBuffer@AsciiString@@IAEXXZ (retail 0x00887940) instead of inlining
...
// -----------------------------------------------------
// SHIM: set(const AsciiString&) left undefined so operator= emits a call to
// ?set@AsciiString@@QAEXABV1@@Z (retail 0x00887C90, the body the ledger also
// knows as ?set@?$StringBase@D@@QAEXABV1@@Z).
