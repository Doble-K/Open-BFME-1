...
protected:
	...
	// TU-scoped shim (Code/GameEngineDevice/Source/W3DDevice/GameClient/Water/W3DWater.cpp only):
	// retail addVelocity's `m_disableRiver = true` proves this field sits at
	// this+0x2c5, i.e. 12 bytes (3 DWORDs) earlier than the ZH-natural offset
	// this struct's plain declaration order would give it. ReAcquireResources'
	// retail bytes (masm dump, 3 consecutive pointer field accesses at
	// this+0x2b0/+0x2b4/+0x2b8 = m_whiteTexture/m_waterNoiseTexture/
	// m_waterSparklesTexture) confirm retail dropped exactly 3 DWORDs from
	// right here (the per-instance pixel-shader-handle trio) - ReAcquireResources'
	// retail body no longer creates per-instance pixel shaders at all. Moving
