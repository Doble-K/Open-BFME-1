...
// Real name from reverse/symbols.csv. Follows ZH's shape -- ignore a frame not
// newer than m_disconnectFrames[playerID], then resetPlayersVotes, store the
// frame, set the received flag -- with two BFME changes: an unconditional
// conMgr->sendFrameDataToPlayer(playerID, frame) straight after the flag, and a
// three-argument resend (playerID, theirFrame, currentFrame) in the remote
// branch where ZH passes two. The ZH C++ reproduces everything up to that last
// call, which is why this is asm rather than a body in DisconnectManager.cpp.
__declspec(naked) void DisconnectManager::processDisconnectFrame(NetCommandMsg *msg, ConnectionManager *conMgr)
{
	__asm {
		push ebp
		push esi
		...
		__emit 018h
		__emit 09Dh
		__emit 0FFh   // call 0x3D5F0
		cmp dword ptr [edi+ebp*4+230h], eax
		jae L00_66BE97
		push ebx
		...
		__emit 09Dh
		__emit 0FFh   // call 0x3D5F0
		mov dword ptr [edi+ebp*4+230h], eax
		mov ecx, esi
		mov byte ptr [edi+ebp+250h], 1h
		__emit 0E8h
		__emit 01Ch
		__emit 018h
		__emit 09Dh
...
L04_66BE11:
		...
		__emit 0E8h
		__emit 09Ch
		__emit 0FFh   // call 0x3A648
		cmp al, 1h
		jne L02_66BE4A
		mov ecx, dword ptr [esp+14h]
		mov eax, dword ptr [ecx]
		cmp eax, dword ptr [edi+ebp*4+230h]
		jae L02_66BE4A
		cmp byte ptr [esi+edi+250h], 1h
		jne L02_66BE4A
		__emit 08Bh
		__emit 015h
		__emit 098h
...
L02_66BE4A:
		...
		ret 8h
L01_66BE62:
		mov esi, dword ptr [edi+ebp*4+230h]
		mov ecx, ebx
		__emit 0E8h
		__emit 021h
		__emit 0E4h
		__emit 09Dh
		__emit 0FFh   // call 0x4A291
		cmp esi, dword ptr [edi+eax*4+230h]
		jae L07_66BE96
		cmp byte ptr [edi+ebp+250h], 1h
		jne L07_66BE96
		__emit 08Bh
		__emit 015h
		__emit 098h
...
// Real name from reverse/symbols.csv, and the last of the three named-but-
// unmatched DisconnectManager bodies. Its first half is the reference's --
// getDisconnectSlot, then disconnectPlayer -- and BFME appends blame assignment:
// with m_pingsSent at +0x260 positive it divides m_pingsRecieved at +0x264 by it
// and, when the ratio falls under the constant at 0x01075C70, sets a flag at
// TheGameLogic+0x290. That tail is x87 throughout, so this is bytes rather than
// C++.
__declspec(naked) void DisconnectManager::processDisconnectPlayer(NetCommandMsg *msg, ConnectionManager *conMgr)
{
	__asm {
		...
		__emit 0E9h
		__emit 09Dh
		__emit 0FFh   // call 0x4B0B0
		mov ecx, dword ptr [esi+260h]
		test ecx, ecx
		mov dword ptr [esp+0Ch], ecx
		jle L00_66C7E8
		fild dword ptr [esi+264h]
		fidiv dword ptr [esp+0Ch]
		__emit 0D8h
		__emit 01Dh
		__emit 070h
		__emit 05Ch
...
L01_66C7B5:
		...
		__emit 03Bh
		__emit 008h
		__emit 001h   // fcomp dword ptr [0x1083b6c]
		fnstsw ax
		test ah, 5h
		jp L00_66C7E8
		__emit 0A1h
		...
		__emit 02Fh
		__emit 001h   // mov eax, dword ptr [0x12f0898]
		cmp dword ptr [eax+290h], 1h
		je L00_66C7E8
		mov dword ptr [eax+290h], 0h
L00_66C7E8:
		pop esi
		ret 8h
	}
